...
import ProjectDetail from "./components/ProjectDetail"; // Import ajouté
import "./i18n/config";

function App() {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [activeTab, setActiveTab] = useState("dashboard");
  const [selectedProjectId, setSelectedProjectId] = useState<number | null>(null); // État ajouté

  // Fonction modifiée pour gérer les projets
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className={`transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-16'}`}>
        ...
        <main className="p-6 overflow-x-hidden">
          ...
          {activeTab === 'projects' && (
            <ProjectsPage onNavigate={handleTabChange} />
          )}
          {activeTab === 'new-project' && (
            <NewProject onNavigate={handleTabChange} />
          )}
          {activeTab === 'analytics' && <AnalyticsPage />}
          {activeTab === 'project-detail' && selectedProjectId && ( // Section ajoutée
            <ProjectDetail 
